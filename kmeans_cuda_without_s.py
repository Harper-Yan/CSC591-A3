...
extern "C" 
__global__ void compute_distances(float* data, float* centriods, float* distances)
{
  ...
  if(i < 1000 && j < 50){
    float d=0;
    d+=(data[i*2]-centriods[j*2])*(data[i*2]-centriods[j*2]);
    d+=(data[i*2+1]-centriods[j*2+1])*(data[i*2+1]-centriods[j*2+1]);    
    d=sqrt(d);                     
    distances[i*50+j]=d;                                      
  }
...
def assign_to_clusters(data, centroids, block_dim=(32, 32,1)):
    ...
    grid_dim = (centroids.shape[0],1,1)

    # Launch the CUDA kernel
    compute_distances_kernel(grid_dim, block_dim,(data_gpu, centroids_gpu, distances_gpu))
...
log_file_name = 'kmeans_cuda_execution_log.txt'
...
plt.title('K-Means Clustering Cuda')
plt.xlabel('Feature 1')
plt.ylabel('Feature 2')
plt.legend()
plt.savefig("K-Means Clustering Cuda.png")
plt.show()
